...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_moonlander(
    ...
    MT(MOD_RCTL, KC_TAB),KC_R,           KC_S,           KC_T,           KC_H,           KC_D,           OSL(1),                                                                         KC_TRANSPARENT, KC_M,           KC_N,           KC_A,           KC_E,           KC_O,           KC_QUOTE,       
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case ST_MACRO_2:
    if (record->event.pressed) {
      SEND_STRING(SS_TAP(X_W)SS_DELAY(25)  SS_TAP(X_A)SS_DELAY(25)  SS_TAP(X_N)SS_DELAY(25)  SS_TAP(X_T)SS_DELAY(25)  SS_TAP(X_SPACE));
    }
    break;
    case ST_MACRO_3:
    if (record->event.pressed) {
